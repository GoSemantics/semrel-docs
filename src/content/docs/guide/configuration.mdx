...
Controls what semrel does when the computed next version tag already exists locally (e.g. after a manual tag push or a re-run).
...
| `phase` | string | no | When to run: `condition` (before any git work) or `release` (after tag created, default) |

*Either `uses` or `path` must be set.

### Plugin phases

The `phase` field controls *when* in the release pipeline the plugin runs:

| Phase | When it runs | Typical use |
| --- | --- | --- |
| `condition` | After config load, **before** any commit analysis or tag creation | `condition-github-actions`, `condition-gitlab-ci` — abort if environment is wrong |
| `release` | After the tag is created (default) | `provider-github`, `hook-slack`, `updater-npm` — publish and notify |
...
plugins:
  ...
  # Post-release: create GitHub release with changelog body
  - uses: provider-github
    phase: release   # this is the default, can be omitted
    args:
      token: ${{ env.GITHUB_TOKEN }}
```
