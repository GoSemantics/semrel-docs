...
2. The plugin listens on a randomly chosen local port and prints it to stdout.
3. semrel connects and calls the appropriate RPCs in pipeline order.
4. After the pipeline completes, semrel terminates all plugin processes.

Because the transport is gRPC/Protobuf, plugins can be written in **any language** — Go, Rust, Python, Node.js, etc.
