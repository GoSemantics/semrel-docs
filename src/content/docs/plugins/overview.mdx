...
semrel's release pipeline is composed of standalone plugin executables. Each plugin is discovered locally and executed as a subprocess — there is no gRPC layer and no `hashicorp/go-plugin` handshake.
...
The current official plugin catalog is organized into five categories.
