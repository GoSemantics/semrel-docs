...
Discover plugins in the [Plugin Registry](/plugins/registry/).
...
<Aside type="tip">
  Commit your `.semrel/` directory to version control so that your team and CI always use the same plugin versions. If a plugin is missing locally, semrel can also discover it through the [Plugin Registry](/plugins/registry/), download the matching binary, and verify its checksum before caching it.
</Aside>
